...
Starting with :class:`~puchkidb.queries.Query` you can construct complex
queries:
...
class Query(QueryImpl):
    ...
    Allows to build queries for PuchkiDB databases. There are two main ways of
    using queries:
    ...
    def _generate_test(self, test, hashval):
        ...
        :return: A :class:`~puchkidb.queries.QueryImpl` object
        """
        if not self._path:

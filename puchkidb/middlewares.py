"""
Contains the :class:`base class <puchkidb.middlewares.Middleware>` for
middlewares and implementations.
"""
from . import PuchkiDB
...
class Middleware(object):
    ...
    Middlewares hook into the read/write process of PuchkiDB allowing you to
    ...
    ``_storage_cls`` (see :class:`~puchkidb.middlewares.CachingMiddleware` for an
    example).
    """

    def __init__(self, storage_cls=PuchkiDB.DEFAULT_STORAGE):
    ...
    def __call__(self, *args, **kwargs):
        ...
        Usually a user creates a new PuchkiDB instance like this::

            PuchkiDB(storage=StorageClass)

        The storage kwarg is used by PuchkiDB this way::
        ...
        Using Middlewares, the user will call::
                                       ...
                                       v
            PuchkiDB(storage=Middleware(StorageClass))
                       ^
                       Already an instance!

        So, when running ``self.storage = storage(*args, **kwargs)`` Python
        now will call ``__call__`` and PuchkiDB will expect the return value to
        ...
        from PuchkiDB (``PuchkiDB(arg1, kwarg1=value, storage=...)``).
...
class CachingMiddleware(Middleware):
    """
    Add some caching to PuchkiDB.

    This Middleware aims to improve the performance of PuchkiDB by writing only

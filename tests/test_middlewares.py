...
from puchkidb.storages import MemoryStorage, JSONStorage
...
def test_caching_read():
    db = PuchkiDB(storage=CachingMiddleware(MemoryStorage))
    assert db.all() == []
...
def test_caching_json_write(tmpdir):
    ...
    with PuchkiDB(path, storage=CachingMiddleware(JSONStorage)) as db:
        db.insert({'key': 'value'})

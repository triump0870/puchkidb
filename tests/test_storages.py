...
from puchkidb.database import Document
from puchkidb.storages import JSONStorage, MemoryStorage, Storage, touch
...
def test_json_kwargs(tmpdir):
    ...
    db = PuchkiDB(str(db_file), sort_keys=True, indent=4, separators=(',', ': '))
...
def test_json_readwrite(tmpdir):
    ...
    # Create PuchkiDB instance
    db = PuchkiDB(path, storage=JSONStorage)
...
def test_json_invalid_directory():
    with pytest.raises(IOError):
        with PuchkiDB('/this/is/an/invalid/path/db.json', storage=JSONStorage):
            pass
...
def test_in_memory_close():
    with PuchkiDB(storage=MemoryStorage) as db:
        db.insert({})
...
def test_read_once():
    ...
    with PuchkiDB(storage=MyStorage) as db:
        reset_counter()
...
def test_yaml(tmpdir):
    ...
    # Write contents
    path = str(tmpdir.join('test.db'))
    db = PuchkiDB(path, storage=YAMLStorage)
    db.insert(doc)
    assert db.all() == [doc]

...
from puchkidb.middlewares import Middleware
...
def test_insert_valid_mapping_type(db):
    from puchkidb.database import Mapping
...
def test_cutom_mapping_type_with_json(tmpdir):
    ...
    # Insert
    db = PuchkiDB(str(tmpdir.join('test.db')))
    db.purge()
    db.insert(CustomDocument({'int': 1, 'char': 'a'}))
    assert db.count(where('int') == 1) == 1
...
def test_multiple_dbs():
    ...
    """
    db1 = PuchkiDB(storage=MemoryStorage)
    db2 = PuchkiDB(storage=MemoryStorage)
...
def test_storage_closed_once():
    ...
    with PuchkiDB(storage=Storage) as db:
        db.close()
...
def test_gc(tmpdir):
    # See https://github.com/msiemens/puchkidb/issues/92
    path = str(tmpdir.join('db.json'))
    db = PuchkiDB(path)
    table = db.table('foo')
...
def test_non_default_table():
    ...
    assert [PuchkiDB.DEFAULT_TABLE] == list(db.tables())

    db = PuchkiDB(storage=MemoryStorage, default_table='non-default')
    assert {'non-default'} == db.tables()

    db.purge_tables()
    default_table = PuchkiDB.DEFAULT_TABLE

    PuchkiDB.DEFAULT_TABLE = 'non-default'
    db = PuchkiDB(storage=MemoryStorage)
    assert {'non-default'} == db.tables()

    PuchkiDB.DEFAULT_TABLE = default_table
...
def test_purge_table():
    ...
    db.purge_table(PuchkiDB.DEFAULT_TABLE)
    assert [] == list(db.tables())

    table_name = 'some-other-table'
    db = PuchkiDB(storage=MemoryStorage)
    db.table(table_name)
    assert {PuchkiDB.DEFAULT_TABLE, table_name} == db.tables()

    db.purge_table(table_name)
    assert {PuchkiDB.DEFAULT_TABLE} == db.tables()
...
def test_empty_write(tmpdir):
    ...
    PuchkiDB(path).close()
    PuchkiDB(path, storage=ReadOnlyMiddleware()).close()
...
def test_query_cache():
    ...
    db._table_cache[PuchkiDB.DEFAULT_TABLE]._read = lambda: {}
...
def test_puchkidb_is_iterable(db):
...
def test_custom_table_class():
    from puchkidb.database import Table
    ...
    db = PuchkiDB(storage=MemoryStorage)
    assert isinstance(PuchkiDB(storage=MemoryStorage).table(),
                      Table)
    assert isinstance(db.table('my_table', table_class=MyTableClass),
                      MyTableClass)

    # Table class for all tables
    PuchkiDB.table_class = MyTableClass
    assert isinstance(PuchkiDB(storage=MemoryStorage).table(),
                      MyTableClass)
    assert isinstance(PuchkiDB(storage=MemoryStorage).table('my_table'),
                      MyTableClass)

    # Reset default table class
    PuchkiDB.table_class = Table


def test_string_key():
    from puchkidb.database import Table, StorageProxy, Document
    from puchkidb.storages import MemoryStorage
    ...
    db = PuchkiDB(storage=MemoryStorage, table_class=Table2,
...
def test_string_key2():
    ...
    PuchkiDB.storage_proxy_class = StorageProxy2
    db = PuchkiDB(storage=MemoryStorage, table_class=Table2)
    table = db.table()
    table.insert({'doc_id': 'abc'})
    assert table.get(doc_id='abc')['doc_id'] == 'abc'
